...
func (e *EthRPCService) GasPrice(ctx context.Context) (result string, err error) {
	...
	rpcRes, rpcErr := client.Call("dnero.GetBlockByHeight", trpc.GetBlockByHeightArgs{Height: currentHeight})
	...
	dneroGetBlockResult, ok := resultIntf.(common.DneroGetBlockResult)
	if !ok {
		return "", fmt.Errorf("failed to convert GetBlockResult")
	}
	totalGasPrice := big.NewInt(0)
	count := 0
	for _, tx := range dneroGetBlockResult.Txs {
		if types.TxType(tx.Type) != types.TxSmartContract {
			continue
		}
		if tx.Tx != nil {
