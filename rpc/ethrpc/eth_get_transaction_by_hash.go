...
func (e *EthRPCService) GetTransactionByHash(ctx context.Context, hashStr string) (result common.EthGetTransactionResult, err error) {
	...
	for i := 0; i < maxRetry; i++ { // It might take some time for a block to be finalized, retry a few times
		...
		if (dneroGetTransactionResult.BlockHash != tcommon.Hash{}) {
			break
		}
	...
	nativeTxHash := dneroGetTransactionResult.TxHash // need use native tx hash to find the tx index, instead of the ETH tx hash
	if dneroGetTransactionResult.Tx != nil {
		if types.TxType(dneroGetTransactionResult.Type) == types.TxSend {
			tx := dneroGetTransactionResult.Tx.(*types.SendTx)
		...
		if types.TxType(dneroGetTransactionResult.Type) == types.TxSmartContract {
			tx := dneroGetTransactionResult.Tx.(*types.SmartContractTx)
			...
			result.Nonce = hexutil.Uint64(tx.From.Sequence) - 1 // off-by-one: Ethereum's account nonce starts from 0, while Dnero's account sequnce starts from 1
			GetRSVfromSignature(data, &result)
		}
	}
