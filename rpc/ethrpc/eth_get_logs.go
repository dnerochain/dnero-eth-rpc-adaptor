...
func (e *EthRPCService) GetLogs(ctx context.Context, args EthGetLogsArgs) (result []EthGetLogsResult, err error) {
	...
	maxRetry := 5
	blocks := []*common.DneroGetBlockResultInner{}
...
func retrieveBlockByHash(blockhash tcommon.Hash, blocks *[](*common.DneroGetBlockResultInner), maxRetry int) (err error) {
	parse := func(jsonBytes []byte) (interface{}, error) {
		//logger.Infof("eth_getLogs.parse, jsonBytes: %v", string(jsonBytes))

		trpcResult := common.DneroGetBlockResult{DneroGetBlockResultInner: &common.DneroGetBlockResultInner{}}
	...
	var block common.DneroGetBlockResult
	for i := 0; i < maxRetry; i++ { // It might take some time for a tx to be finalized, retry a few times
		...
		if err == nil {
			block = resultIntf.(common.DneroGetBlockResult)
			break
		}

		logger.Warnf("eth_getLogs, dnero.GetBlock returned error: %v", err)
		time.Sleep(blockInterval) // one block duration
	}

	if block.DneroGetBlockResultInner != nil {
		(*blocks) = append((*blocks), block.DneroGetBlockResultInner)
	}

	return nil
}

func retrieveBlocksByRange(fromBlock string, toBlock string, blocks *[](*common.DneroGetBlockResultInner), maxRetry int) (err error) {
	parse := func(jsonBytes []byte) (interface{}, error) {
		//logger.Infof("eth_getLogs.parse, jsonBytes: %v", string(jsonBytes))

		trpcResult := common.DneroGetBlocksResult{}

		var objList []json.RawMessage
		json.Unmarshal(jsonBytes, &objList)
		for _, blockJsonBytes := range objList {
			getBlockResult := common.DneroGetBlockResult{DneroGetBlockResultInner: &common.DneroGetBlockResultInner{}}
			...
			trpcResult = append(trpcResult, getBlockResult.DneroGetBlockResultInner)
		}
	...
	// if blockStart >= 2 {
	// 	blockStart -= 2 // Dnero requires two consecutive committed blocks for finalization
	// }
	...
	for i := 0; i < maxRetry; i++ { // It might take some time for a tx to be finalized, retry a few times
		...
		rpcRes, rpcErr := client.Call("dnero.GetBlocksByRange", trpc.GetBlocksByRangeArgs{Start: tcommon.JSONUint64(blockStart), End: tcommon.JSONUint64(blockEnd)})
		resultIntf, err := common.HandleDneroRPCResponse(rpcRes, rpcErr, parse)
		if err != nil {
			blocks = &[]*common.DneroGetBlockResultInner{}
			logger.Warnf("eth_getLogs, dnero.GetBlocksByRange returned error: %v", err)
			time.Sleep(blockInterval) // one block duration
			continue
		}

		getBlocksRes := resultIntf.(common.DneroGetBlocksResult)
...
func extractLogs(addresses []tcommon.Address, topicsFilter [][]tcommon.Hash, filterByAddress bool, blocks [](*common.DneroGetBlockResultInner), result *([]EthGetLogsResult)) {
